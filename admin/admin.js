...
(function () {
  ...
  async function saveModule(key, row, event) { const button = event.currentTarget.querySelector('button[type="submit"]'); const data = new FormData(event.currentTarget); const payload = {}; button.disabled = true; button.textContent = 'Saving...'; modules[key].forEach(([name,,type]) => { payload[name] = type === 'checkbox' ? (!row && name === 'is_published' ? true : data.has(name)) : type === 'number' ? Number(data.get(name) || 0) : String(data.get(name) || '').trim(); }); if (key === 'projects') payload.technologies = normalizeArrayField(payload.technologies); if (key === 'services') payload.features = normalizeArrayField(payload.features); const formStatus = $('formStatus'); for (const url of ['live_url','github_url']) if (payload[url] && !/^https?:\/\//i.test(payload[url])) { if (formStatus) { formStatus.textContent = 'URLs must begin with http:// or https://.'; } button.disabled = false; button.textContent = 'Save'; return; } if (key === 'projects' && !row) { const duplicate = await client.from('projects').select('id').eq('slug', payload.slug).maybeSingle(); if (duplicate.error) { if (formStatus) { formStatus.textContent = showError(duplicate.error); } button.disabled = false; button.textContent = 'Save'; return; } if (duplicate.data) { if (formStatus) { formStatus.textContent = 'That slug already exists.'; } button.disabled = false; button.textContent = 'Save'; return; } } const result = row ? await client.from(key).update(payload).eq('id', row.id) : await client.from(key).insert([{ ...payload, created_by: state.user.id }]).select().single(); if (result.error) { if (formStatus) { formStatus.textContent = showError(result.error); formStatus.className = 'status full error'; } button.disabled = false; button.textContent = 'Save'; return; } closeDialog(); toast('Saved successfully.'); await show(key); }
  ...
  (async () => { 
    ...
    if (!state.user) return location.replace('../index.html#admin-login'); 
    ...
    window.setInterval(checkUnreadMessages, 30000); 
  ...
  function setAdminTheme(theme) {
    ...
    if (themeToggle) {
      themeToggle.innerHTML = theme === 'light'
        ? '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M21 12.8A9 9 0 0 1 11.2 3a9 9 0 1 0 9.8 9.8Z"></path></svg>'
        : '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M12 3v2m0 14v2m9-9h-2M5 12H3m14.95 5.95-1.41-1.41M8.46 8.46 7.05 7.05m9.9 0-1.41 1.41M8.46 15.54l-1.41 1.41"></path><circle cx="12" cy="12" r="4"></circle></svg>';
  ...
  function initAdminTheme() {
    ...
    console.log('Theme system initialized with:', saved);
  }
}());
