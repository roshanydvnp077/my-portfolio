(function () {
  ...
  function memberCard(member) {
    ...
    return '<article class="family-card"><div class="family-card-head">' + avatar + '<div class="family-card-identity"><h3>' + escape(member.full_name) + '</h3><p>' + escape(member.relation || 'Family member') + '</p></div><span class="family-status"><i></i>Active</span></div><div class="family-details">' + '<div class="family-detail"><span>📅 <b>Date of Birth</b></span><strong>' + escape(member.date_of_birth ? dateText(member.date_of_birth) : 'Not provided') + '</strong></div>' + '<div class="family-detail"><span>📄 <b>Documents</b></span><strong>' + docs.length + (docs.length === 1 ? ' document' : ' documents') + '</strong></div>' + '<div class="family-detail"><span>📞 <b>Phone</b></span><strong>' + escape(member.phone || 'Not provided') + '</strong></div>' + '</div><div class="family-actions">' + action('button primary', '↗', 'Open Vault', 'data-vault-open') + action('button', '✎', 'Edit', 'data-vault-edit') + action('button danger', '⌫', 'Delete', 'data-vault-delete') + action('button', '⇩', 'Download Details', 'data-vault-download-member') + action('button', '⌕', 'View Details', 'data-vault-view-member-details') + action('button', '⎙', 'Print Details', 'data-vault-print-member') + '</div></article>';
  }
