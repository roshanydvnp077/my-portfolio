  ...
  function renderTestimonials() {
    ...
    client.from('testimonials').select('*').eq('is_published', true).order('display_order', { ascending: true }).order('sort_order', { ascending: true }).order('created_at', { ascending: false }).then(result => {
      ...
      const render = () => {
        ...
        target.innerHTML = visible.map(row => { const name = row.full_name || row.name || ''; const image = resolveUrl('portfolio-images', row.profile_image || row.photo); const role = row.role || row.position || ''; const company = row.company || ''; const review = row.review || row.message || ''; const rating = Math.max(1, Math.min(5, Number(row.rating || 5))); const profile = row.website_url || row.linkedin_url || ''; return '<article data-cms-content="testimonials" class="testimonial-card" tabindex="0" role="button" aria-expanded="false"><div class="testimonial-client testimonial-client-top">' + (image ? '<img class="testimonial-client-avatar" src="' + escape(image) + '" alt="' + escape(name) + '" loading="lazy">' : '<span class="testimonial-client-avatar">' + escape(initials(name)) + '</span>') + '<div class="testimonial-client-info"><strong>' + escape(name) + '</strong></div></div><div class="testimonial-details"><span class="testimonial-quote-mark">“</span><p class="testimonial-review">' + escape(review) + '</p><div class="testimonial-stars" aria-label="' + rating + ' out of 5 stars">' + '★'.repeat(rating) + '<span>' + '★'.repeat(5 - rating) + '</span></div>' + (role || company ? '<p class="testimonial-extra">' + escape([role, company].filter(Boolean).join(' · ')) + '</p>' : '') + (profile ? '<a class="testimonial-profile-link" href="' + escape(profile) + '" target="_blank" rel="noopener">View profile</a>' : '') + '</div></article>'; }).join('');
      ...
      const toggleCard = card => { if (!card) return; const expanded = card.classList.toggle('is-expanded'); card.setAttribute('aria-expanded', String(expanded)); };
      target.addEventListener('click', event => { if (event.target.closest('a')) return; toggleCard(event.target.closest('.testimonial-card')); });
      target.addEventListener('keydown', event => { if ((event.key === 'Enter' || event.key === ' ') && !event.target.closest('a')) { event.preventDefault(); toggleCard(event.target.closest('.testimonial-card')); } });
      render();
    });
  }
  async function load() {
