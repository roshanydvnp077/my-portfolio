  ...
  function renderTestimonials() {
    ...
    client.from('testimonials').select('*').eq('is_published', true).order('display_order', { ascending: true }).order('sort_order', { ascending: true }).order('created_at', { ascending: false }).then(result => {
      ...
      const render = () => {
        ...
        target.innerHTML = visible.map(row => { const name = row.full_name || row.name || ''; const image = resolveUrl('portfolio-images', row.profile_image || row.photo); const role = row.role || row.position || ''; const review = row.review || row.message || ''; const rating = Math.max(1, Math.min(5, Number(row.rating || 5))); return '<article data-cms-content="testimonials" class="testimonial-card"><div class="testimonial-client testimonial-client-top">' + (image ? '<img class="testimonial-client-avatar" src="' + escape(image) + '" alt="' + escape(name) + '" loading="lazy">' : '<span class="testimonial-client-avatar">' + escape(initials(name)) + '</span>') + '<div class="testimonial-client-info"><strong>' + escape(name) + '</strong><span>' + escape([role, row.company].filter(Boolean).join(' · ')) + '</span>' + (row.website_url || row.linkedin_url ? '<a href="' + escape(row.website_url || row.linkedin_url) + '" target="_blank" rel="noopener">View profile</a>' : '') + '</div></div><span class="testimonial-quote-mark">“</span><p class="testimonial-review">' + escape(review) + '</p><div class="testimonial-stars" aria-label="' + rating + ' out of 5 stars">' + '★'.repeat(rating) + '<span>' + '★'.repeat(5 - rating) + '</span></div></article>'; }).join('');
